...
export default function GameCard({ game, selected, onToggle }) {
  ...
  return (
    <div onClick={onToggle} style={{
      background: selected ? 'rgba(34,197,94,0.06)' : 'var(--bg4)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            <div style={{ width: 30, height: 30, borderRadius: 7, background: away.color || 'var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <span style={{ fontSize: 9, fontWeight: 900, color: 'white', fontFamily: 'DM Mono, monospace' }}>{game.away}</span>
            </div>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text1)', lineHeight: 1.2 }}>{away.city} {away.name}</div>
              <div style={{ fontSize: 10, color: 'var(--text4)' }}>Away</div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ width: 30, height: 30, borderRadius: 7, background: home.color || 'var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <span style={{ fontSize: 9, fontWeight: 900, color: 'white', fontFamily: 'DM Mono, monospace' }}>{game.home}</span>
            </div>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text1)', lineHeight: 1.2 }}>{home.city} {home.name}</div>
              <div style={{ fontSize: 10, color: 'var(--text4)' }}>{game.time} · {game.net}</div>
            </div>
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
          ...
          <div style={{
            ...
            border: `2px solid ${selected ? '#22c55e' : 'var(--border2)'}`,
