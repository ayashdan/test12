...
function GamesTab({ picks }) {
  ...
  // Flatten all user picks across weeks for game id lookup
  const allUserPicks = {}
  Object.values(picks).forEach(data => {
    Object.assign(allUserPicks, data.picks || {})
  })
  ...
  return (
    <>
        ...
        {weeks.map(w => {
          ...
          return (
            <button key={w} onClick={() => setSelectedWeek(w)} style={{
              flexShrink: 0,
              background: isActive ? 'rgba(59,130,246,0.18)' : 'var(--bg2)',
              border: `1px solid ${isActive ? '#3b82f6' : 'var(--border)'}`,
              borderRadius: 16, padding: '5px 12px',
              fontSize: 12, fontWeight: isActive ? 700 : 500,
              color: isActive ? '#60a5fa' : 'var(--text3)',
      ...
      {loading && Array.from({ length: 3 }).map((_, i) => (
        <div key={i} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 14, padding: '14px', marginBottom: 8, opacity: 0.6 }}>
          <div style={{ height: 12, background: 'var(--bg3)', borderRadius: 6, width: '40%', marginBottom: 10 }} />
          <div style={{ display: 'flex', gap: 8 }}>
            <div style={{ flex: 1, height: 48, background: 'var(--bg3)', borderRadius: 10 }} />
            <div style={{ width: 48, height: 48, background: 'var(--bg3)', borderRadius: 10 }} />
            <div style={{ flex: 1, height: 48, background: 'var(--bg3)', borderRadius: 10 }} />
          </div>
        </div>
      ))}
      ...
      {/* Error state */}
      {!loading && error && (
        <div style={{ textAlign: 'center', padding: '32px 20px', background: 'var(--bg2)', borderRadius: 14, border: '1px solid #ef444440', marginBottom: 12 }}>
          <div style={{ fontSize: 28, marginBottom: 8 }}>📡</div>
          <div style={{ fontWeight: 700, color: '#ef4444' }}>Schedule unavailable</div>
          <div style={{ fontSize: 12, color: 'var(--text4)', marginTop: 6 }}>{error}</div>
        </div>
      )}
      ...
      {!loading && games.map(game => {
        const userPick = allUserPicks[game.id]
        ...
        const awayPct = game.awayWinPct
        const homePct = game.homeWinPct
        const showWinBar = isPre && awayPct != null && homePct != null

        return (
          <div key={game.id} style={{
            background: 'var(--bg2)',
            border: `1px solid ${isLive ? '#22c55e44' : 'var(--border)'}`,
            borderRadius: 14, padding: '12px 14px', marginBottom: 10, position: 'relative',
          }}>
            {/* Top row: date + network badge */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text2)' }}>{game.azDate}</div>
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                {isLive && (
                  <span style={{ fontSize: 9, fontWeight: 800, background: '#ef4444', color: 'white', borderRadius: 4, padding: '2px 7px', letterSpacing: '0.05em' }}>● LIVE {game.statusText}</span>
                )}
                {isFinal && (
                  <span style={{ fontSize: 9, fontWeight: 700, background: 'var(--bg3)', color: 'var(--text4)', borderRadius: 4, padding: '2px 6px' }}>FINAL</span>
                )}
                {game.network && (
                  <span style={{ fontSize: 9, fontWeight: 800, background: 'rgba(59,130,246,0.15)', color: '#60a5fa', borderRadius: 4, padding: '2px 8px', letterSpacing: '0.04em' }}>{game.network}</span>
                )}
              </div>
            </div>

            {/* Teams row */}
            <div style={{ display: 'flex', alignItems: 'stretch', gap: 0, borderRadius: 10, overflow: 'hidden', marginBottom: 8 }}>
              {/* Away side */}
              <div style={{ flex: 1, background: awayColor + '22', borderLeft: `3px solid ${awayColor}`, padding: '10px 12px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                <div style={{ fontSize: 22, fontWeight: 900, color: awayColor, fontFamily: "'DM Mono',monospace", letterSpacing: '0.04em' }}>{game.away.abbr}</div>
                <div style={{ fontSize: 10, color: 'var(--text4)', marginTop: 1 }}>{game.away.city}</div>
              </div>

              {/* Center: score / countdown / time */}
              <div style={{ minWidth: 72, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'var(--bg3)', padding: '8px 6px' }}>
                {(isLive || isFinal) ? (
                  <>
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      <span style={{ fontSize: 20, fontWeight: 900, color: game.away.score >= game.home.score ? 'var(--text1)' : 'var(--text4)', fontFamily: "'DM Mono',monospace" }}>{game.away.score ?? '–'}</span>
                      <span style={{ fontSize: 13, color: 'var(--border2)' }}>–</span>
                      <span style={{ fontSize: 20, fontWeight: 900, color: game.home.score >= game.away.score ? 'var(--text1)' : 'var(--text4)', fontFamily: "'DM Mono',monospace" }}>{game.home.score ?? '–'}</span>
                    </div>
                    {isLive && <div style={{ fontSize: 9, color: '#ef4444', fontWeight: 700, marginTop: 2 }}>{game.statusText}</div>}
                  </>
                ) : game.countdown ? (
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: '#22c55e', fontFamily: "'DM Mono',monospace", lineHeight: 1.3 }}>
                      {game.countdown.days > 0 ? `${game.countdown.days}d ` : ''}{game.countdown.hours}h {game.countdown.minutes}m
                    </div>
                    <div style={{ fontSize: 9, color: 'var(--text4)', marginTop: 2 }}>until kickoff</div>
                  </div>
                ) : (
                  <div style={{ fontSize: 11, color: 'var(--text4)', fontWeight: 700 }}>{game.azTimePart}</div>
                )}
              </div>

              {/* Home side */}
              <div style={{ flex: 1, background: homeColor + '22', borderRight: `3px solid ${homeColor}`, padding: '10px 12px', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', justifyContent: 'center' }}>
                <div style={{ fontSize: 22, fontWeight: 900, color: homeColor, fontFamily: "'DM Mono',monospace", letterSpacing: '0.04em' }}>{game.home.abbr}</div>
                <div style={{ fontSize: 10, color: 'var(--text4)', marginTop: 1 }}>{game.home.city}</div>
              </div>
            </div>
            ...
            {showWinBar && (
              <div style={{ marginBottom: 6 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 10, fontWeight: 700, color: awayColor }}>{Math.round(awayPct)}%</span>
                  <span style={{ fontSize: 9, color: 'var(--text4)' }}>Win Probability (ESPN FPI)</span>
                  <span style={{ fontSize: 10, fontWeight: 700, color: homeColor }}>{Math.round(homePct)}%</span>
                </div>
              </div>
            )}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 4 }}>
              <div style={{ fontSize: 10, color: 'var(--text4)' }}>
                ...
                {game.spread && game.overUnder != null && <span style={{ margin: '0 4px' }}>·</span>}
                {game.overUnder != null && <span>O/U {game.overUnder}</span>}
                {!game.spread && game.overUnder == null && <span>{game.azTimePart} AZ</span>}
              </div>
              {userPick && (
                <span style={{ fontSize: 9, fontWeight: 700, background: '#22c55e20', color: '#22c55e', border: '1px solid #22c55e40', borderRadius: 4, padding: '2px 8px' }}>
                  Your pick: {userPick} ✓
                </span>
              )}
            </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 16px 12px', position: 'sticky', top: 0, zIndex: 10, background: 'var(--bg)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ position: 'relative', width: 44, height: 44 }}>
            <span style={{ position: 'absolute', top: -8, left: -4, fontSize: 9, fontWeight: 900, color: '#22c55e', fontFamily: 'DM Mono,monospace', opacity: 0.9 }}>21</span>
